...
> **維護者**：工程團隊
>
> **v1.2.0 變更摘要**：依 2026-03-11 業主確認稿完成 MVP 對齊，定案四角色同平台（caregiver/patient/provider/admin）、被照護者簡化介面納入 MVP、新增服務人員工作介面規格、定案 8 個服務入口分類、補齊平台介入式媒合與最小雙向確認流程、補齊服務人員等級制度與 API/驗收規範。
...
| 被照護者介面 | Patient UI | 被照護者本人的簡化行動介面（同平台登入入口），可查看自身健康摘要、提醒與近期行程，權限為唯讀子集 |

### A.4 MVP 定義

**核心閉環**：委託人登入 → 管理被照護者 → 紀錄健康數據 → 查看趨勢/AI 放心報 → 從 8 個服務入口送出需求 → 營運方進行平台介入式媒合（候選篩選 + 最小雙向確認）→ 指派服務人員 → 服務人員回報進度 → 委託人與被照護者追蹤狀態。

**MVP 平台形態定案**：

- 單一平台、單一登入入口，依角色顯示不同介面
- MVP 四種角色皆為核心交付：`caregiver`、`patient`、`provider`、`admin`
- 被照護者與服務人員介面為同平台角色化畫面，不拆成第二套獨立產品線

**成功指標**（可量測）：

| 指標 | 目標值 |
|------|--------|
| 核心閉環端到端可跑通 | 100%（Demo 劇本 12 步驟全部通過） |
| API 回應時間（P95） | < 500ms（排除 AI 呼叫） |
| AI 放心報生成時間 | < 10 秒 |
| App 冷啟動到首頁 | < 3 秒 |
| 系統可用性（Staging 環境） | > 99%（以 Vercel 保證為準） |
| 四角色關鍵流程驗收 | 100%（caregiver/patient/provider/admin 劇本全部通過） |
| 8 服務入口可操作率 | 100%（8 類別皆可建立需求單） |

### A.5 範圍界線

#### MVP 必做（10 模組）

1. 帳號系統 + 四角色權限（caregiver/patient/provider/admin）
2. 健康數據紀錄（血壓/血糖）
3. 趨勢圖表 + 數據分析呈現
4. AI 放心報（健康摘要）
5. 提醒與通知
...
- **金流**：收款/分潤/退款/對帳/發票
- **自由接案 marketplace**：搶單/競標、即時協商、即時地圖與定位追蹤
...
│  │  (Expo RN)   │          │   /admin/*            │  │
│  │  caregiver/patient/provider │   營運方端         │  │
│  └──────┬───────┘          └──────────┬───────────┘  │
│         │ HTTPS                       │ HTTPS        │
└─────────┼─────────────────────────────┼──────────────┘
          │                             │
...
委託人手動輸入健康數據
  ...
  Admin 後台查看需求單清單
        │
        ▼
  Admin 提出候選服務人員 → 委託人確認 → 服務人員確認 → 自動 arranged
        │
        ▼
...
MVP 階段定案為四種角色（同平台登入入口）：

| 角色 | 英文 | 說明 |
|------|------|------|
| 委託人 | `caregiver` | Mobile App 使用者，管理自己的被照護者 |
| 被照護者 | `patient` | Mobile App 簡化介面使用者，只能查看與自己綁定的資料 |
| 服務人員 | `provider` | Mobile App 工作介面使用者，查看平台分派案件並回報進度 |
| 管理員 | `admin` | Web Admin 使用者，管理全平台資源與媒合流程 |

> 角色切換規則：同一帳號僅有單一主角色；若需跨角色操作，必須建立獨立帳號並透過後台建立關聯。

### C.2 權限矩陣

| 資源 | 動作 | `caregiver` | `patient` | `provider` | `admin` |
|------|------|:-----------:|:---------:|:----------:|:-------:|
| 自己的帳號 | 讀取/更新 | ✅ | ✅ | ✅ | ✅ |
| 被照護者（Recipients） | 建立 | ✅（自己的） | ❌ | ❌ | ✅ |
| 被照護者 | 讀取 | ✅（自己的） | ✅（僅綁定本人） | ❌ | ✅（全部） |
| 被照護者 | 更新 | ✅（自己的） | ❌ | ❌ | ✅（全部） |
| 健康數據（Measurements） | 建立 | ✅（自己被照護者的） | ❌ | ❌ | ✅ |
| 健康數據 | 讀取 | ✅（自己被照護者的） | ✅（僅綁定本人） | ❌（僅可讀案件摘要） | ✅ |
| AI 放心報 | 生成 | ✅（自己被照護者的） | ❌ | ❌ | ✅ |
| AI 放心報 | 讀取 | ✅（自己被照護者的） | ✅（僅綁定本人） | ❌ | ✅ |
| 行程（Appointments） | 建立/更新/刪除 | ✅（自己被照護者的） | ❌ | ❌ | ✅ |
| 行程（Appointments） | 讀取 | ✅（自己被照護者的） | ✅（僅綁定本人） | ✅（僅被指派案件） | ✅ |
| 服務需求單 | 建立 | ✅（自己的） | ❌ | ❌ | ✅（代建） |
| 服務需求單 | 讀取 | ✅（自己的） | ✅（僅綁定本人） | ✅（僅被指派案件） | ✅（全部） |
| 服務需求單 | 更新狀態（媒合） | ❌ | ❌ | ✅（僅服務執行進度） | ✅ |
| 服務需求單 | 指派服務人員 | ❌ | ❌ | ❌ | ✅ |
| 通知 | 讀取/標已讀 | ✅（自己的） | ✅（自己的） | ✅（自己的） | ✅（自己的） |
| 服務人員（Providers） | 讀取/更新本人資料 | ❌ | ❌ | ✅（本人） | ✅（全部） |
| 服務人員（Providers） | 審核/等級調整 | ❌ | ❌ | ❌ | ✅ |
| 服務項目（Categories） | 讀取入口 | ✅ | ✅ | ✅ | ✅ |
| 服務項目（Categories） | 管理 | ❌ | ❌ | ❌ | ✅ |
...
```typescript
// 虛擬碼：依角色做 ownership gate
async function checkOwnership(role: string, userId: string, requestId: string): Promise<boolean> {
  if (role === 'caregiver') {
    return !!(await prisma.recipient.findFirst({ where: { id: requestId, caregiver_id: userId, deleted_at: null } }));
  }
  if (role === 'patient') {
    return !!(await prisma.recipient.findFirst({ where: { id: requestId, patient_user_id: userId, deleted_at: null } }));
  }
  // provider 需檢查 assigned_provider_id 對應關係
  return false;
}
```

- **Caregiver 流程**：`recipient_id` 必須屬於當前 `caregiver`
- **Patient 流程**：`recipient.patient_user_id` 必須等於當前 `patient` 的 `user_id`
- **Provider 流程**：`service_request.assigned_provider_id` 必須對應到當前 `provider` 的 profile
- **Notifications**：`notifications.user_id` 必須等於當前使用者
...
| `role` | `varchar(20)` | NOT NULL, default `'caregiver'` | `caregiver` / `patient` / `provider` / `admin` |
...
| `patient_user_id` | `uuid` | FK → users.id, UNIQUE, NULL | 綁定被照護者本人帳號（Patient 角色） |
...
索引：`INDEX(caregiver_id)` , `INDEX(caregiver_id, deleted_at)` , `UNIQUE(patient_user_id) WHERE patient_user_id IS NOT NULL`
...
|------|------|------|------|
| `id` | `uuid` | PK | 主鍵 |
| `code` | `varchar(40)` | NOT NULL, UNIQUE | 穩定代碼（不可隨意變更） |
...
MVP 固定 8 類服務入口（`code` → `name`）：

1. `escort_visit` → `陪診師`
2. `pre_visit_consult` → `診前諮詢`
3. `shopping_assist` → `購物服務`
4. `exercise_program` → `運動項目`
5. `home_cleaning` → `打掃清潔`
6. `daily_living_support` → `生活輔助`
7. `nutrition_consult` → `營養表諮詢`
8. `functional_assessment` → `身體功能檢測`
...
| `candidate_provider_id` | `uuid` | FK → providers.id, NULL | 媒合候選服務人員（平台提出） |
| `assigned_provider_id` | `uuid` | FK → providers.id, NULL | 指派的服務人員 |
| `caregiver_confirmed_at` | `timestamptz` | NULL | 委託人確認候選時間 |
| `provider_confirmed_at` | `timestamptz` | NULL | 服務人員確認接案時間 |
| `provider_note` | `text` | NULL | 服務人員處理備註 |
...
- `submitted`（委託人送出）
- `screening`（營運審查需求）
- `candidate_proposed`（平台提出候選服務人員）
- `caregiver_confirmed`（委託人確認候選）
- `provider_confirmed`（服務人員確認）
- `arranged`（已安排）
- `in_service`（服務進行中）
- `completed`（服務完成）
- `cancelled`（取消；`completed` 以外任何狀態可取消）
...
|------|------|------|------|
| `id` | `uuid` | PK | 主鍵 |
| `user_id` | `uuid` | FK → users.id, UNIQUE, NULL | 綁定 provider 角色登入帳號 |
| `name` | `varchar(100)` | NOT NULL | 姓名 |
| `phone` | `varchar(20)` | NULL | 電話 |
| `email` | `varchar(255)` | NULL | 信箱 |
| `level` | `varchar(10)` | NOT NULL, default `'L1'` | 服務等級（`L1` / `L2` / `L3`） |
...
| `availability_status` | `varchar(20)` | NOT NULL, default `'available'` | `available` / `busy` / `offline` |
...
索引：`INDEX(review_status)` , `INDEX(level)` , `UNIQUE(user_id) WHERE user_id IS NOT NULL`
...
// 3. 被照護者帳號：patient.demo@remotecare.dev / Patient1234!（role=patient）
// 4. 服務人員帳號：provider.demo@remotecare.dev / Provider1234!（role=provider）
// 5. 兩位被照護者：「王奶奶」（綁定 patient 帳號）、「李爺爺」（未綁定）
// 6. 各 30 筆血壓/血糖歷史資料（過去 30 天，含 3 筆異常）
// 7. 兩筆就醫行程（未來 7 天、14 天各一筆）
// 8. 八個固定服務類別（escort_visit...functional_assessment）
// 9. 至少三筆服務需求單（submitted/candidate_proposed/arranged 各一）
// 10. 兩位服務人員（含 level 與 availability；一位 approved 綁定 provider 帳號）
// 11. 五筆通知（含 caregiver/patient/provider 三角色）
// 12. 一筆 AI 放心報
```
...
- 角色：`caregiver`、`patient`、`provider`、`admin`
- 成功回應（200）：
```json
{
  "success": true,
...
- 角色：`caregiver`（只看自己的）、`patient`（只回傳綁定自己的一筆）、`admin`（全部，支援 `?caregiver_id=` 篩選）
- 成功（200）：
```json
{
  "success": true,
...
- 角色：`caregiver`（ownership 檢查）、`patient`（僅可讀綁定本人）、`admin`
...
- 角色：`caregiver`（ownership）、`patient`（僅綁定本人）、`admin`
- 必要參數：`recipient_id`
...
- 角色：`caregiver`（ownership）、`patient`（僅綁定本人）
...
- 角色：`caregiver`、`patient`、`provider`（皆只讀自己的）
- 參數：`is_read`（可選）、`page`、`limit`
- 預設排序：`created_at DESC`

**`GET /api/v1/notifications/unread-count`** — 取得未讀數量

- 角色：`caregiver`、`patient`、`provider`
- 成功（200）：`{ "success": true, "data": { "count": 5 } }`

**`PUT /api/v1/notifications/:id/read`** — 標記已讀

- 角色：`caregiver`、`patient`、`provider`（ownership）

**`PUT /api/v1/notifications/read-all`** — 全部已讀

- 角色：`caregiver`、`patient`、`provider`

**`GET /api/v1/recipients/:id/reminders`** — 取得被照護者的量測提醒設定

- 角色：`caregiver`（ownership）
- 成功（200）：提醒陣列，每筆含 `id`、`reminder_type`（`morning` / `evening`）、`reminder_time`（`HH:mm`）、`is_enabled`

**`PUT /api/v1/recipients/:id/reminders/:type`** — 更新提醒設定

- 角色：`caregiver`（ownership）
- `:type` = `morning` | `evening`
- Request：`{ "reminder_time": "08:30", "is_enabled": true }`（皆為可選）
- 成功（200）：更新後的提醒物件
- 備註：提醒由 `POST /recipients` 自動建立（見 F.3.2），不提供獨立 POST / DELETE
...
- 角色：`caregiver`（ownership）、`patient`（僅綁定本人）、`provider`（僅可看被指派案件行程）、`admin`
- 必要參數：`recipient_id`
...
- 角色：
  - `caregiver`：只看自己送出的需求
  - `patient`：只看與自己綁定 recipient 相關的需求（唯讀）
  - `provider`：只看分派給自己的需求
  - `admin`：可看全部，支援 `?status=`、`?category_code=`、`?provider_id=` 篩選
...
{
  "status": "screening",
  "admin_note": "需求資訊完整，進入候選媒合"
}
```
- 狀態轉換規則必須驗證（見 D.3 狀態機）

**`PUT /api/v1/service-requests/:id/propose-candidate`** — 提出候選服務人員（Admin）
...
{
  "provider_id": "uuid",
  "admin_note": "候選人具陪診經驗與同區域服務能力"
}
```
- 驗證：provider 必須 `review_status = 'approved'`
- 成功後狀態改為 `candidate_proposed`，並通知 caregiver 與 provider

**`PUT /api/v1/service-requests/:id/confirm-caregiver`** — 委託人確認候選
...
```
- `confirm=true`：狀態改為 `caregiver_confirmed`
- `confirm=false`：狀態回到 `screening`，並清空候選 provider

**`PUT /api/v1/service-requests/:id/confirm-provider`** — 服務人員確認案件
...
```
- `confirm=true`：狀態改為 `provider_confirmed`
- `confirm=false`：狀態回到 `screening`
- 當 `caregiver_confirmed` + `provider_confirmed` 皆成立時，系統自動設定：
  - `assigned_provider_id = candidate_provider_id`
  - `status = arranged`
  - 送出需求更新通知給 caregiver / patient / provider

**`PUT /api/v1/service-requests/:id/provider-progress`** — 服務人員更新執行進度

- 角色：`provider`（僅 assigned provider）
- 可更新狀態：`arranged -> in_service -> completed`
- Request：
```json
{
  "status": "in_service",
  "provider_note": "已抵達服務地點，開始陪診"
}
```

---

#### F.3.8 服務人員（Providers）
...
- 角色：`admin`
- 篩選：`?review_status=`、`?level=`、`?availability_status=`、`?search=`（姓名模糊搜尋）
...
**`GET /api/v1/provider/me`** — Provider 取得本人資料與工作摘要

- 角色：`provider`
- 回傳內容：provider profile、level、availability、待處理案件數、今日行程摘要

**`PUT /api/v1/provider/me`** — Provider 更新本人可維護欄位

- 角色：`provider`
- 可更新欄位：`phone`、`availability_status`、`service_areas`、`admin_note`（僅追加工作說明，不可覆寫審核備註）
...
#### F.3.10 Provider Workspace

**`GET /api/v1/provider/tasks`** — 取得服務人員任務列表

- 角色：`provider`
- 支援篩選：`?status=`（`arranged` / `in_service` / `completed`）
- 只回傳 `assigned_provider_id` 為當前 provider 的案件

**`GET /api/v1/provider/tasks/:id`** — 取得任務詳情

- 角色：`provider`（僅 assigned provider）
- 回傳：案件內容、被照護者必要資訊、行程資訊、歷次備註

**`PUT /api/v1/provider/tasks/:id/report`** — 回報處理進度
...
├── (patient-tabs)/
│   ├── _layout.tsx
│   ├── summary/index.tsx     # 個人健康摘要（唯讀）
│   ├── schedule/index.tsx    # 提醒與近期行程（唯讀）
│   └── notifications.tsx     # 個人通知
├── (provider-tabs)/
│   ├── _layout.tsx
│   ├── tasks/index.tsx       # 任務清單
│   ├── tasks/[id].tsx        # 任務詳情
│   ├── tasks/[id]/report.tsx # 回報進度
│   └── profile/index.tsx     # 服務人員個人資料/待辦摘要
...
- **互動**：
  - 表單驗證：email 格式、密碼非空
  - 登入成功 → 儲存 token → 依角色導向（caregiver/patient/provider/admin）
  - 登入失敗 → 顯示錯誤訊息（inline）
- **Loading**：按鈕 spinner
...
- **目的**：新委託人註冊（MVP 僅開放 caregiver 自助註冊）
- **UI 區塊**：
...
- **列表頁**：
  - 需求單列表，依狀態分色：
    - `submitted`（灰）→ `screening`（藍）→ `candidate_proposed`（靛）→ `caregiver_confirmed`（青）→ `provider_confirmed`（綠）→ `arranged`（深綠）→ `in_service`（橘）→ `completed`（綠）→ `cancelled`（紅）
  - 每筆：服務類別、被照護者、期望日期、狀態 badge
  - 點擊 → 詳情頁

- **新增頁**：
  - 被照護者選擇器
  - 8 個服務入口卡片（從 API 動態取得並固定順序）
    - 陪診師、診前諮詢、購物服務、運動項目、打掃清潔、生活輔助、營養表諮詢、身體功能檢測
  - 期望日期（date picker）
  - 時段偏好（早上/下午/晚上）
  - 地點（文字輸入）
...
- **詳情頁**（`services/[requestId].tsx`）：
  - 完整需求資訊
  - 狀態時間軸（顯示各狀態轉換時間）
  - 當狀態為 `candidate_proposed` 時顯示「同意候選服務人員」操作（caregiver）
  - 指派的服務人員資訊（若已指派）：姓名、專業
  - 營運方備註（若有）

- **API**：`GET /api/v1/service-categories`、`POST /api/v1/service-requests`、`GET /api/v1/service-requests`、`GET /api/v1/service-requests/:id`、`PUT /api/v1/service-requests/:id/confirm-caregiver`
...
#### G.2.12 被照護者摘要頁（`(patient-tabs)/summary/index.tsx`）

- **目的**：被照護者以最小唯讀介面查看自己的近況
- **UI 區塊**：
  - 今日摘要卡：最近血壓/血糖、異常提示、下一個提醒
  - 近 7 天簡化趨勢圖（唯讀）
  - 最近一筆服務需求狀態（若有）
- **限制**：不可新增/編輯量測、不可送出需求單
- **API**：`GET /api/v1/recipients`、`GET /api/v1/measurements/stats`、`GET /api/v1/service-requests`

#### G.2.13 被照護者提醒與行程頁（`(patient-tabs)/schedule/index.tsx`）

- **目的**：查看與本人相關的提醒、通知與近期安排
- **UI 區塊**：
  - 提醒清單（量測/行程/服務進度）
  - 未來 30 天行程列表（唯讀）
  - 服務進度摘要卡（顯示目前狀態）
- **API**：`GET /api/v1/notifications`、`GET /api/v1/appointments`、`GET /api/v1/service-requests`

#### G.2.14 服務人員任務列表（`(provider-tabs)/tasks/index.tsx`）

- **目的**：查看平台分派案件與個人待辦摘要
- **UI 區塊**：
  - 狀態篩選（arranged / in_service / completed）
  - 任務卡：服務類型、被照護者、日期時段、地點、目前狀態
  - 待處理數量摘要
- **API**：`GET /api/v1/provider/tasks`

#### G.2.15 服務人員任務詳情與回報（`(provider-tabs)/tasks/[id].tsx`）
...
- **UI 區塊**：
  ...
  - 備註輸入區（處理回報）
  - 提交後顯示時間戳與成功提示
- **API**：`GET /api/v1/provider/tasks/:id`、`PUT /api/v1/provider/tasks/:id/report`、`PUT /api/v1/service-requests/:id/confirm-provider`
...
Given：Admin 在後台提出候選服務人員
When：委託人在需求詳情點選「同意候選」
Then：狀態更新為「委託人已確認」
...
```
Given：被照護者帳號已綁定 recipient
When：以 patient 角色登入
Then：顯示個人摘要、提醒與近期行程，且所有輸入控制為唯讀
```

#### 劇本 6：服務人員工作介面

```
Given：需求單已為 arranged 且已指派給 provider
When：provider 進入任務詳情並更新為 in_service，再更新為 completed 並填寫備註
Then：案件狀態正確更新，委託人與被照護者可在需求進度看到最新回報
```
...
- **列表頁**：
  ...
  - 篩選：狀態 dropdown、服務類別、候選/指派服務人員、關鍵字搜尋
  - 分頁
  - 點擊 → 詳情頁
...
- **詳情頁**：
  ...
  - 提出候選服務人員（dropdown：`approved` + `availability_status='available'` 的 provider）
  - 追蹤雙向確認狀態（caregiver/provider）
  - 營運備註輸入
  - 操作歷程（顯示 `updated_at`）
...
- **列表頁**：
  - 表格：姓名、等級、電話、專業、證照、服務區域、可接案狀態、審核狀態、建立日期
  - 篩選：審核狀態、等級、可接案狀態
  - 「＋ 新增」按鈕
...
包含：2 位被照護者、各 30 筆量測紀錄（含異常）、2 筆行程、8 個服務類別、3 筆需求單（含媒合狀態差異）、2 位服務人員（含 level）、4 角色通知、1 筆 AI 放心報。
...
| 10 | 6:00-7:00 | Web Admin | 以 admin 登入 → Dashboard 總覽 → 查看需求單 → 提出候選服務人員 | 營運後台閉環 |
| 11 | 7:00-7:30 | Mobile | caregiver 同意候選 → provider 確認接案 → 需求單自動進入「已安排」 | 雙向確認閉環 |
| 12 | 7:30-8:00 | Mobile | 以 provider 帳號登入 → 查看待辦案件 → 回報「服務完成」 | 四角色閉環 |
...
| 模組 | 驗收條件 |
|------|---------|
| 1. 帳號系統 | 註冊/登入/登出正常；JWT 驗證正確；四角色導流正確；ownership 檢查通過 |
...
| 7. 服務需求 | 送出成功；8 入口可用；平台媒合狀態機合法轉換；雙向確認可跑通；非法轉換被拒絕 |
| 8. 管理後台 | Dashboard 數據正確；需求單管理完整；候選提案與媒合追蹤正常 |
| 9. 服務人員 | 建檔/審核/等級調整正確；文件上傳成功；Provider 工作介面可更新進度與備註 |
...
> 以下僅列 v1.0 上線後方向；四角色同平台（caregiver/patient/provider/admin）、被照護者簡化介面、服務人員工作介面、8 服務入口與平台媒合流程均已納入 MVP，不再列為延後項目。

### MVP v1.x 優化方向（上線後小版本）

- 服務媒合規則優化（候選排序策略、地區/時段匹配權重）
- Provider 回報欄位模板化（依服務類型顯示不同欄位）
- 被照護者摘要可讀性優化（字級、圖卡樣式、提醒分群）

### Phase 2（其他項目）

- 硬體串接：藍牙血壓計/血糖機 SDK 整合
- Push Notification（FCM/APNs）
- 獨立服務人員專用 App（非同平台角色化介面；含打卡與定位）
- 金流整合（綠界/藍新）
- 評價系統
- 個人化異常閾值

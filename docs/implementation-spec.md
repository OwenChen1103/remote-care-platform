...
| 被照護者介面 | Patient-Lite | 被照護者本人的簡化行動介面，資訊範圍小於委託人介面，具體可見資料與可用功能待定（見 Q 節） |
...
> MVP 階段不新增其他角色。後續已確認將規劃被照護者本人的簡化介面（Patient-Lite），其身份模型可能採用新增 `patient` 角色或其他等效方案，具體設計待定（見 Q 節）。
...
> 以下僅列方向，**不屬於 MVP 核心 Slice（0-6）範圍**。標示「已確認方向」者為客戶已確認之產品需求，將於 MVP 後期或 Phase 2 初期排入。

### Patient-Lite 被照護者介面（已確認方向）

客戶已確認被照護者本人將擁有自己的行動端介面，資訊範圍小於委託人介面。此功能規劃為 MVP 後期 Slice（見 implementation-plan.md），但以下設計細節尚未定案：

**初步方向（待確認）**：
- 同一個 Mobile App，依角色或身份顯示不同介面
- 被照護者介面的資訊範圍與可用操作為委託人介面的子集
- 具體可見資料、可執行動作、身份模型皆待定

**待決事項（須於實作前逐項確認）**：

| 項目 | 選項 | 目前傾向 |
|------|------|---------|
| 被照護者帳號建立方式 | 委託人邀請 / Admin 建立 / 自行註冊 | 待定 |
| 身份模型 | 新增 `patient` role / 其他等效方案 | 待定 |
| 被照護者 ↔ 使用者帳號連結方式 | recipients 表加 FK / 獨立關聯表 | 待定 |
| 可見範圍：量測紀錄與趨勢 | 全部 / 部分 / 不可見 | 待定 |
| 可見範圍：AI 報告 | 完整報告 / 僅摘要 / 不可見 | 待定 |
| 可見範圍：服務需求 | 可見狀態 / 不可見 | 待定 |
| 是否可自行記錄量測 | 可以 / 不可以 | 待定 |
| 是否可觸發 AI 報告生成 | 可以 / 不可以 | 待定 |
| 通知範圍 | 量測提醒 / 異常警報 / 行程提醒 / 全部 | 待定 |
| 一位被照護者是否可有多個帳號 | 1:1 / 1:N | 待定 |

> 以上待決事項將在 Patient-Lite Slice 正式規劃時定案。在此之前，不進行相關的 schema 變更、角色新增、或 UI 實作。
